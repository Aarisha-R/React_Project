...
AdditionalQuestions.propTypes = {
  addQuestionData: PropTypes.func.isRequired,
};
...
export default function AdditionalQuestions({ addQuestionData }) {
  ...
  const submit = (e) => {
    ...
    if (!profession || !interest || !reference) {
      ...
      // Copy state values into mutable variables before modifying them
      let professionValue = profession;
      let interestValue = interest;
      let referenceValue = reference;

      if (professionValue === "Others") {
        professionValue = otherProfession;
      }
      if (interestValue === "Others") {
        interestValue = otherInterest;
      }
      if (referenceValue === "Others") {
        referenceValue = otherReference;
      }

      // Log the selected options and call the addQuestionData function with the data
      console.log(professionValue, interestValue, referenceValue);
      addQuestionData(professionValue, interestValue, referenceValue);
